 ...
 * 
 * Thereby all operation will run in constant time <b>O(1)</b> (only some
 * methods like the {@link #filteredBy(CardFilter)} operation will run (at
 * least) in
 * <b>O(n)</b> [with n = number of cards in set]).
 * 
 ...
 * ranked cards in there at least useful as kickers.
 * 
 * @author Jan Bernitt
 * 
 */
public final class CardSet implements Iterable<Card> {
	...
	public CardSet highestNStraightFlush(int n) {
		...
		for (Suit suit : Suit.values()) {
			long suitCards = CardBoard.withAllOf(suit);
			CardSet straightFlush = highestNStraightOf(CardBoard.intersection(setCards, suitCards),
					n);
